...
/// <summary>
/// 스테이지 실패(즉사·완전사망) 시 스탬프를 짧게 보여 준다. StageClearBannerUI와 대칭 — 타이밍·
/// 연출 골격이 동일하고, 다른 이벤트를 구독할 뿐이다.
...
/// </summary>
public class StageFailedBannerUI : MonoBehaviour
{
    [Header("타이밍(초)")]
    ...
    void Subscribe()
    {
        ...
        state.OnAnyStageFailedPulse -= Show;
        state.OnAnyStageFailedPulse += Show;
    }
    ...
    void Unsubscribe()
    {
        ...
        if (StageNetworkState.Instance != null)
            StageNetworkState.Instance.OnAnyStageFailedPulse -= Show;
    }

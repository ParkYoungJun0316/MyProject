...
/// <summary>좌/우 분기 미니게임 한 라운드의 판정 조건. 시드로부터 재생성되는 순수 데이터.</summary>
public struct SideSplitRound
{
    public int leftCount;
    public int rightCount;
    public bool hasColorRequirement;
    public bool colorOnLeft; // hasColorRequirement가 true일 때만 의미 있음
...
///
/// [확정 규칙 — MinigameDesign.md §1.2]
///  - 인원 분배: 활성 플레이어 전원이 반드시 좌/우로 다 나뉨 (남는 인원 없음)
...
///  4. 타이머 종료 시(ServerTime 기준) Host만 좌/우 볼륨 물리 오버랩으로 판정(Judge)
///     - 좌/우 인원이 지정값과 정확히 일치 + (색 조건 있으면) 지정 색 플레이어가 지정 방향에 존재 → 성공
...
public class SideSplitChallenge : MonoBehaviour
{
    ...
    void Start()
    {
        leftZone?.SetState(SideSplitZone.VisualState.Neutral);
        rightZone?.SetState(SideSplitZone.VisualState.Neutral);
    ...
    /// <summary>
    /// 타이머 종료 시 Host만 호출. 좌/우 볼륨 물리 오버랩으로 인원·색상 조건을 판정.
    /// [확정 규칙] 정확히 일치해야 성공 — 양쪽 동시 점유·어느 쪽에도 없는 생존자가 있으면 그 자체로 실패.
    ...
    {
        List<Player> inLeft  = leftZone?.GetPlayersInVolume(playerOverlapLayers)  ?? new List<Player>();
        List<Player> inRight = rightZone?.GetPlayersInVolume(playerOverlapLayers) ?? new List<Player>();

        var leftSet  = new HashSet<Player>(inLeft);
        var rightSet = new HashSet<Player>(inRight);

        int  actualLeft     = 0;
        int  actualRight    = 0;
        bool misplaced      = false; // 양쪽 동시 점유 또는 어느 쪽에도 없는 생존자 존재
        ...
        foreach (Player p in allPlayers)
        {
            ...
            bool inL = leftSet.Contains(p);
            bool inR = rightSet.Contains(p);
            ...
            if (inL) actualLeft++;
            else if (inR) actualRight++;
            else { misplaced = true; continue; }

            if (round.hasColorRequirement && !colorSatisfied)
            {
                bool onRequiredSide = round.colorOnLeft ? inL : inR;
                if (onRequiredSide && p.isUniqueColor && p.playerColorType == round.requiredColor)
                    colorSatisfied = true;
            }
        }

        bool countMatch = !misplaced && actualLeft == round.leftCount && actualRight == round.rightCount;
        bool success    = countMatch && colorSatisfied;

        leftZone?.SetState(success ? SideSplitZone.VisualState.Success : SideSplitZone.VisualState.Fail);
        rightZone?.SetState(success ? SideSplitZone.VisualState.Success : SideSplitZone.VisualState.Fail);
    ...
    ///  3. 라운드 0..totalRounds-1 순서대로: 좌/우 인원 분배 → (색 조건 라운드면) 색 배정 쪽·색상 결정
    ///
    ...
    void RegenerateRoundPlan()
    {
        ...
        {
            int leftCount  = totalPlayers > 0 ? rng.Next(0, totalPlayers + 1) : 0;
            ...
            if (i >= firstColorRoundIndex && totalPlayers > 0)
            {
                ...
                if (leftCount == 0)       round.colorOnLeft = false;
                else if (rightCount == 0) round.colorOnLeft = true;
                else                       round.colorOnLeft = rng.Next(0, 2) == 0;

...
///  3. 라운드 시작(ChallengeStepBegin) → preRoundCountdown초 카운트다운(OnCountdownTick) →
...
///
/// [재배치 — zoneRig] zoneRig가 연결되면 rotationStartRound 라운드부터 라운드 시작(카운트다운 시작)
/// 순간 90도 단위로 스냅한다. 카운트다운 동안은 방향 표시가 가려져 있다가(SideSplitWorldDisplay)
/// 공개 순간 "다른 자리에서 나타나는" 것으로 읽힌다 — 판 4개가 중심 대칭이라 스냅해도 서는 자리는
/// 그대로고 방향 이름만 바뀐다. 각도는 RegenerateRoundPlan()이 시드로 결정(전 머신 동일).
/// (2026-09-22: 라운드 사이 스핀 폐기 — 스핀 중엔 방향을 못 읽고 시작 시점도 안 보였음)
/// </summary>
public class SideSplitChallenge : MonoBehaviour
{
    ...
    [Tooltip("결과 연출 후 다음 라운드(카운트다운 시작)까지 대기 시간(초)")]
    public float resolveDelay = 0f;

    [Tooltip("라운드마다 방향·인원 공개 전 카운트다운(초). 첫 라운드 포함. 이 동안 타이머는 돌지 않는다.")]
    public float preRoundCountdown = 3f;

    [Tooltip("공개(OnRoundReady) 직후 타이머가 줄기 시작하기 전 읽는 시간(초). 이 동안 남은 시간은 제한시간 그대로 유지.")]
    public float revealReadHold = 1f;

    [Tooltip("라운드 실패 시 전원에게 줄 피해량")]
    public int wrongDamage = 1;

    [Header("재배치 — 카운트다운 동안 zoneRig 90도 단위 스냅")]
    [Tooltip("Left/Right/Front/Back SideSplitZone을 자식으로 둔 부모 Transform. 비어 있으면 재배치 비활성.")]
    public Transform zoneRig;

    [Tooltip("이 라운드 인덱스(0-based)부터 재배치 시작. 이전 라운드는 항상 0도 고정.")]
    ...
    [Header("UI 이벤트")]
    [Tooltip("라운드 공개 전 카운트다운 남은 시간(초) — 0.1초 간격. 끝나면 OnRoundReady.")]
    public SideSplitFloatEvent OnCountdownTick;

    [Tooltip("새 라운드 조건 전달(카운트다운이 끝난 순간) → UI에 연결")]
    ...
    void HandleChallengeStepChanged(int stepIndex)
    {
        ...
        // 카운트다운 시작과 동시에 재배치 — 카운트다운 동안 방향 표시가 가려져 있어 공개 순간 새 자리에서 나타난다.
    ...
    /// <summary>현재 판의 라운드 제한시간(초). 표시 측(SideSplitWorldDisplay)이 남은 시간 비율을 계산할 때 사용.</summary>
    public float CurrentRoundTimeLimit => RoundTimeLimit;

    /// <summary>
    /// 이번 판의 라운드 제한시간. 인원수는 GameSession.ActivePlayerCount(전 머신 동일, RegenerateRoundPlan과
    /// 같은 전제)라 타이머·판정 시각이 머신마다 갈리지 않는다.
    /// </summary>
    ...
    /// <summary>
    /// ServerTime 기준 공통 진행: [카운트다운 preRoundCountdown초] → OnRoundReady →
    /// [읽는 시간 revealReadHold초 — 남은 시간 꽉 참] → [제한시간 타이머].
    /// 전 머신이 같은 시점에 공개·타임업을 감지하고, Host만 이어서 실제 물리 판정(Judge)을 수행한다.
    /// </summary>
    IEnumerator TimerRoutine()
    {
        ...
        {
            double stepStart = _netState != null ? _netState.ChallengeStepStartServerTime : 0.0;
            float  left      = preRoundCountdown - (float)((nm != null ? nm.ServerTime.Time : 0.0) - stepStart);
            if (left <= 0f) break;
        ...
        while (_challengeActive)
        {
            // 공개 후 revealReadHold 동안은 elapsed가 0으로 묶여 남은 시간이 꽉 찬 채 유지된다.
            double startTime = (_netState != null ? _netState.ChallengeStepStartServerTime : 0.0) + preRoundCountdown + revealReadHold;
            double elapsed    = System.Math.Max(0.0, (nm != null ? nm.ServerTime.Time : 0.0) - startTime);
    ...
    /// 재배치 미사용 인스턴스(zoneRig 미연결)는 그냥 무시.</summary>

...
- ~~**T.Boss P3 튕김 발판(2026-09-08):** `ContactKnockback.launchMode = VerticalUp`~~ — **[폐기 2026-09-21]** 멀미로 폐기. **`ContactKnockback`의 `VerticalUp` 모드 자체는 남긴다** — T.Stage3(33개)·T.Stage5(2층 진입 발판 4개)가 쓰고 있다. P3 발판은 씬에서만 제거.
- **T.Boss Sphere 컨트롤러 — 코드 됨(2026-09-10 구현 + 같은 날 리뷰로 배선 모델 정정). 아래가 실제 구현 상태.**
  ...
  **1) `AdvancingWall.cs` (순수 추가, 기존 사용처 동작 무변경):**
  - ~~`CompleteCurrentEntryNow()`~~ — **[삭제 2026-09-21]** 2026-09-11 재설계로 `SnapToDistance()`가 대체한 뒤 호출자가 0이 된 채 남아 있었다. P3 폐기 정리에서 같이 지움.
  ...
  **2) `BossSpherePhaseDriver.cs` (신규, `Assets/Scripts/Stage/`):**
  - `AdvanceToCheckpoint(int checkpointIndex)` — 진입 시 `sphereWall.SnapToDistance(앵커)`로 이 칸의 시작 체크포인트로 스냅한 뒤 `RunOnceSynced(바닥까지 남은 거리, timeLimit, 하강 시작 서버 시각)`. 인덱스 범위 밖이면 `LogError`. (구 기술: `CompleteCurrentEntryNow()` + `RunOnce` — 2026-09-11·09-18에 각각 대체됨)
  ...
  **3) 씬 배선 (전부 인스펙터, `PhaseData` 기존 이벤트 재사용 — 이 항목은 코드 아니라 씬 작업):**
  ...
  - **P1 시작 = `StageStartGate.OnCountdownComplete` → `AdvanceToCheckpoint(0)`.**
    ⚠️ **2026-09-21: 이 배선이 씬에서 빠져 있던 것을 발견해 복구했다.** P1을 파괴 타일 트랙으로
    교체할 때 같이 날아간 것으로 보인다 — 그동안 **P1에서는 Sphere 시계가 아예 안 돌았다**
    (`phaseTimeLimits[0]=75`를 쓰는 경로가 없었다). 복구 후 `AdvanceToCheckpoint` 호출은
    `IntArgument` 0/1/2/3 네 개가 전부 정상이다. phase 0의 `onPhaseEnter`에 물면 `PhaseManager.Start()`가 **씬 로드 즉시** `EnterPhase(0)`을 호출하므로 카운트다운 전에 시계가 돈다(`BossFightObjective` 주석이 phase 0 챌린지를 게이트에서 시작하라고 한 것과 같은 이유). 이 게이트 이벤트는 Host/Client 양쪽에서 발동한다.
  ...
  **6) 4페이즈 승리조건 배선 — 코드+씬 됨(2026-09-10, 같은 날 2차 세션):**
  ...
  - **[폐기 2026-09-21]** **P3 (히트 카운트, `BossSpherePhaseDriver` 확장):** `requiredHitCounts[4]`(0=그 페이즈는 히트 카운트 승리조건 없음) + `bossFightObjective` 참조 + `RegisterColorHit()` 추가. `AdvanceToCheckpoint()`가 칸이 바뀔 때마다 내부 히트 카운터를 0으로 리셋. `RegisterColorHit()`: Host 전용 가드(Client에서도 로컬로 불릴 수 있어서 — `ColorWall.HandleContact`가 Host 가드가 없는 기존 설계) + 이미 목표를 채웠으면 무시(같은 프레임에 두 번 `NotifyPhaseCleared()`가 불려 페이즈가 한 칸 스킵되는 경합 방지) + 목표 도달 시 `bossFightObjective.NotifyPhaseCleared()`. 씬 배선: Sphere `ColorWall.OnColorMatch → RegisterColorHit()`, 드라이버 `requiredHitCounts=[0,0,8,0]`(P3만 8, 임시 고정값) + `bossFightObjective=BossFlow`.
  - **[폐기 2026-09-21]** **(2026-09-17 Steam 2~4인 실기 버그 수정 — 위 `RegisterColorHit()`/`OnColorMatch` 배선은 폐기)** 증상: P3에서 색을 맞춰 부딪혀도 인식 안 됨 + Sphere가 한 색(파랑)으로 고정된 채 하강. 원인: ① Sphere 색을 설계에 없던 `WallLineRandomizer`가 로컬 타이머로 칠했고, 하강이 끊김 없이 이어져 색 복귀 시점이 오지 않아 P1 초반 색이 고정 + 머신마다 다른 색. ② `ColorWall.HandleContact`가 머신별 로컬 물리로 정지·색 전환을 실행 — Client는 원격 플레이어가 kinematic이라 남의 히트를 못 보고, 카운트는 Host 물리가 본 충돌만 셈. 수정: 맞힌 본인(Owner)만 보고(`StageNetworkState.ReportBossSphereHit`) → Host `BossSpherePhaseDriver.TryAcceptHitFromHost()`가 칸·히트 번호·색 검증 후 번호 +1 → `BossSphereHitState` NV로 전원이 같은 정지 재생. 색은 `requiredHitCounts>0`인 칸에서만 (시드, 칸, 히트 번호)로 전 머신이 동일 계산(`hitColorPool`, 직전 색 연속 금지), 정지 중엔 Default. 그 외 칸은 Default. 씬: Sphere의 `WallLineRandomizer` 제거, `ColorWall.OnColorMatch → RegisterColorHit` 배선 삭제. 실제 씬 값은 `requiredHitCounts=[0,0,15,0]`(위 8과 다름 — 확인 필요).
  ...
  - **P4 벽 계단식 압박(2026-09-22 확정·코드 됨):** `WallLineRandomizer.useSqueeze`(BossWall_F/B/L/R만 켬 — T1·T3는 끔). 돌진 = 남은 바닥 한 변 × 0.9(100→90, 90→81 …), 속도 30m/s 일정(돌진이 짧아질수록 전진 시간도 준다). 후퇴할 때마다 시작 지점이 한 칸씩 안으로 — 한 칸 = 남은 거리 ÷ 남은 시간에 들어갈 횟수(8~9회, 약 5.6m). **Phase 시작 + 90초에 정확히 피벗 45 전진 = 10×10**, 마지막 돌진 뒤 남은 거리는 90초 정각에 맞춰 천천히 붙는다. 벽마다 첫 돌진 3~7초 시드 랜덤(동시 출발 방지), 간격 4~6초 유지. **색 일치로 멈춰도 계단은 내려간다**(`AdvancingWall.RunSurge`가 원점을 출발 전에 확정 — 구 `RunOnce`+후퇴 비율 0.925는 멈추면 그 회차 순전진이 사라져 잘 맞출수록 안 다가오는 버그였다). "동시에 오면 같은 색" 규칙은 **기각**. **(2026-09-26 수정)** 경고색 대기(`colorLeadBeforeWall`) 중 색 일치 멈춤이 걸리면 `RunSurge`가 무시돼 그 머신만 한 칸 뒤처졌다(마지막 회차면 최종 위치 45 미달) — 돌진 직전에 멈춤 해제를 한 번 더 기다린다.
  - **P4 끼임 즉사(2026-09-26 확정·코드 됨):** 막바지 색 실패 시 캐릭터가 벽에 박히거나 벽 뒤로 빠지는 버그(플레이 확인). 원인 = 네 벽이 계단을 따로 내려와 맞은편이 한 칸 앞서 있으면 90% 돌진이 쉬는 벽을 뚫음 + 앞벽이 z 13에 놓여 앞뒤 폭이 99m였음(사용자가 z 15로 이동 → 네 면 모두 ±50.5, 101m). `ContactKnockback`은 기각 — 벽 기준점이 벽 면보다 35m 안쪽이라 벽 쪽으로 튕기고, 끼인 틈에 공간을 못 만든다. **판정(`WallCrushKill`, `Assets/Scripts/Stage`):** 마주 보는 짝 벽(좌↔우, 앞↔뒤 — 전진 방향 내적 < −0.5, 모서리 제외)에 같은 물리 스텝에 둘 다 닿고 **두 벽 모두 내 색 아님**(쉬는 벽 Default 회색 포함) → 즉사. 하나라도 내 색이면 그 벽이 멈추고 후퇴하므로 기존대로(틀린 벽 ContactDamage만). 유예 없음(30m/s = 스텝당 0.6m라 기다리면 이미 박힘). Owner가 판정 → `NetworkPlayerSetup.ReportCrushDeathServerRpc` → Host `NetworkDamageUtil.ApplyInstantKill` → 자동 부활. 벽 돌진·ColorWall·ContactDamage 로직은 무변경(`AdvancingWall.WorldMoveDirection`, `ColorWall.IsColorMatch` 공개만). **에디터 됨:** BossWall_F/B/L/R에 `WallCrushKill` 부착 확인(2026-09-26). **남음:** 플레이 검증(Host+Client).
  - **엔딩 = 사탕 내리찍기 → 바닥 붕괴 → 추락(2026-09-24 확정·코드 됨):** `TBossEndingCollapse`(`Assets/Scripts/Stage`). 흐름: P4 클리어 → Bossdown 대화(4번 "으아아..." 삭제, 대사 개정 쪽에서 처리) → 대화 종료 `OnAllReady` → `Play()` → 사탕이 `windupHeight`만큼 들렸다가 `slamDuration`에 내리찍어 밑면이 바닥 윗면+`headClearance`(플레이어 키 위)에서 멈춤 → 그 순간 `Ground (5)` SetActive(false) + 벽(`enclosingWalls`) 안쪽 사각형만 격자로 `FloorTileShards` 파편·`Breakable_Destroy` → `OnImpact` → `SceneFlowRelay.LoadNextScene`(전환 대기 2.5초 동안 추락이 보임). P4 Ground는 통판이라 칸 파괴(BreakTile/GridTile) 대신 판정 한 번에 끄고 보이는 부분만 파편으로 쪼갠다. 새 RPC·NV 없음(OnAllReady가 전 머신 ClientRpc). 추락 중 낙사(Die 애니·사망)는 **의도적으로 그대로 둔다**(사용자 확정) — 실패·리로드는 `BeginStageResetOnServer`의 AllPhasesComplete/IsTransitioning 가드가 막음. 사탕 콜라이더는 켜진 채(카메라 SphereCast 당김 유지) — 그래서 headClearance로 머리 위에서 멈춘다. `TileDebrisUtil.SpawnAt`(위치·재질 직접 지정) 추가. **에디터 됨(2026-09-24, MCP):** `BossFlow/BossSpherePhaseDriver`에 컴포넌트 — candy=`BackGround/Candy`, floor=`P4/StageManager/Ground (5)`, enclosingWalls=BossWall_L/R/F/B BoxCollider(시작 안쪽 면 x ±50.5, z −50.5/+48.5), debrisPrefab=`FloorTileShards`. Bossdown `OnAllReady` = `Play`만(기존 `LoadNextScene` 제거), `OnImpact` → `SceneFlowRelay.LoadNextScene`. **남음:** 플레이 검증(Host+Client) — 바닥 밑 BackGround 충돌체 유무, 추락 카메라.
...
> **⚠️ 지형 갱신 (2026-09-19 사용자 확정 — §H.2 잠금 해제).**
> **색 패드·다리 문을 T4에서 전면 제거했다.** "패드(Door_3 커먼, 1:1)를 밟으면 Door가 올라와
> 구멍 위 길이 된다"는 더 이상 유효하지 않다. 바닥은 **격자(용량 타일 + 파괴 타일)** 가 대신하며,
> 패드→Door 길은 그 격자와 역할이 중복되어 난이도가 아니라 복잡도만 올렸다.
> **SSOT = [`TStage4TrapRandomization.md`](TStage4TrapRandomization.md).**
>
> **co-op floor는 바뀌지 않는다.** 패드→Door 길은 아래 §2가 "각자 생존"으로 분류해 둔
...
| T1·T3 | 조임 **원상 복구** |
| T2·T4 | **안개 걷힘**(거리 기반) |
| T.Boss P3 | 조임 **원상 복구**(T1·T3와 동일 `EsophagusSqueeze`, 2026-09-10 확정). 판이 2026-09-21에 폐기됐지만 **식도 동사·응원은 그대로 조임** |
...
2인 T2·T4: 한 명이 외치지 않으면 안개가 걷히지 않음. **T4의 2인 장면은 이 외침이다.** 넉백·낙사·바닥 판단은 각자 생존(2026-09-19: "패드→Door 길"이 제거되며 그 자리를 격자 바닥이 대신한다 — §1 T.Stage4 참고).
...
> **⚠️ P3 폐기 (2026-09-21 사용자 확정).** 아래 P3 관련 기술은 전부 기록이다.
>
> **버린 것:** Sphere에 `ColorWall` 부착 · 튕김 발판(`ContactKnockback.VerticalUp`) ·
> 색 히트 보고(`StageNetworkState.ReportBossSphereHit`) · 히트 카운트 승리조건
> (`requiredHitCounts`) · 히트 색 추첨(`hitColorPool`) · 히트 후 정지/재개.
>
> **이유 둘.** ① **멀미** — 발판이 초속 55m/s 수직 발사(정점 154m, 왕복 11초)인데다
> 바닥 100×100 전체가 발판이라 지면에 서 있는 순간이 0초였다. ② **진행 곡선 부재** —
> `PauseTemporarily`가 페이즈 앵커로 완전 복귀시켜서 15번째 시도가 1번째와 물리적으로
> 동일했다. 게다가 `hitColorPool`에 흑백이 있어 한 명이 15번을 다 칠 수 있었다(1인 게임).
>
> **남긴 것:** Sphere 하강 자체 · 페이즈별 체크포인트/시간제한 · 하강 시각 동기화.
> **Sphere는 이제 어떤 페이즈와도 상호작용하지 않는 순수 시계다**(P3만 Sphere를 직접
> 만지는 "의도된 비대칭"도 같이 폐기 — 그 제약이 새 판 아이디어를 막는다는 판단).
>
> **⚠️ 그래서 P3에는 지금 클리어 조건이 없다.** 새 판이 정해질 때까지 P3는 시간 초과로만
> 끝난다. 임시 오브젝티브를 얹지 않기로 확정(2026-09-21) — 정해지면 그때 갈아끼운다.

> **✅ 새 P3 (2026-09-23 사용자 확정 — 09-22 "M5 색 칸 + 처음부터 전부 붕괴" 안을 대체): 숨 쉴 공용 칸.**
> **목표 = survive(N라운드 버티기).** P4(연속 90초, 좁혀지는 공간)와는 **박자형**으로 갈린다 — 라운드마다
> "밖이 무너지는 순간 → 안전 칸에서 숨 쉬기"를 반복한다. §4 초출 금지와 맞음(그리드·붕괴 = M5, 1칸 1명 = T4 용량 타일).
>
> - **판:** 원점 25×25 = 5m 칸 5×5(M.Stage5와 같은 칸·간격). 원점 20×20 전제는 그리드 칸이 대신 지킨다 —
>   P3 시작과 라운드 사이엔 전 칸이 멀쩡하다. 옛 P3 바닥 `Ground (1)`~`(7)`은 **비활성**(삭제 아님).
> - **사이클 = M5와 동일:** 휴식 → 붕괴 구간(무작위 박자 파괴 + 자동 복구, 8방향 연결 보장) → 라운드 시간에 안전 칸 공개
>   (공개 후에도 박자 파괴 계속) → 정산 → 멈춤 → 일괄 복구 → 휴식. 안전 칸은 **처음부터 보여주지 않는다.**
> - **M5와 다른 점은 둘뿐:**
>   1. **안전 칸 = 공용 칸, 1명 전용.** 색 없음(누구나 들어감). 칸 위 2명 이상이면 그 칸의 안전 연출이 꺼지고,
>      1명으로 줄면 다시 켜진다. **2명 이상인 채 정산 = 그 칸 위 전원 개인 데미지 2**(M5 정산 데미지 그대로,
>      나중에 들어온 사람만이 아니라 원래 있던 사람도). 경계에 걸쳐 두 칸에 등록된 사람은 **양쪽 칸에 센다** —
>      두 칸 모두 겹침이 될 수 있고 그대로 둔다(사용자 확정). 침강(`CapacityTile`)은 **안 쓴다.**
>      안전 칸 수 = **인원 + 여유 → 후반 딱 인원**(여유 칸 수는 라운드 커브, 튜닝). 솔로에선 1명 규칙이 증발 — **감수**(T4·P1과 같은 판단).
>   2. **정산 1초 전 안전 칸 외 전부 동시 파괴**(경고는 그보다 0.8초 앞) → 정산 후 M5처럼 일괄 복구.
...
> - **튜닝(2026-09-26, 씬 값):** **6라운드** — 여유 칸 +2(1R) / +1(2~3R) / +0(4~6R), 공개 4.5(1~3R) / 4(4~5R) / 3.5(6R),

...
>
> **2026-09-14 확정 [Ship Must] — 팀 응원 1회 통과.** 구 규칙(첫 인식 후 10초 안에 전원 미달이면 표 전부 리셋)은 **폐기**. 창이 열려 있는 동안 한 번 인식되면 그 사람은 통과·재외침 없음. 성공 조건은 그대로 **전원 각자 1회**. 플레이어가 배우는 규칙은 **§2.1**, 시스템 표는 `CheerSystemDesign.md` §2.2. **코드 완료.**
...
- [x] 구역 4 보드(`Board_GotoStartZone`): **삭제** — 노란 시작 존이 이미 보이고 제목은 지시문이라 불필요. `TutorialGatherZone` 게이트 동작은 그대로. (2026-09-14)
...
| 마이크 청취 **[2026-09-15]** | 각자 PC의 음성 인식도 **배너가 떠 있고 내가 아직 통과 안 했을 때만** 마이크를 듣는다. 외치는 도중 인식되면 바로 통과(말 끝난 뒤 침묵을 기다리지 않음). 상세 `CheerSystemDesign.md` §4.7. |
...
- **표시 [2026-09-22 개정]:** 접시·하트 패널 폐기. 존(`GatherZone` — `SphereCollider` + `TutorialGatherZone`)에 **도착 지점과 같은 발판** `ArrivalPad` 프리팹(`ZonePadVisual`: 크림색 테두리 + 반투명 안쪽 + 위로 투명해지는 빛 벽 + 올라가는 반짝이)을 붙인다. 모이는 곳·ReachZone(T1~T4·T.Boss)·T5 Goal/Goal2F가 전부 같은 모양 — 인원 표시는 하지 않는다. `GatherSign`은 "Start" 간판만 남고, 전원이 모이면 간판 글씨 대신 3·2·1(`TutorialGatherDisplay`, `TutorialNetworkManager.OnGateCountdown*` 구독 — 판정·진행은 그대로 Host). 화면 카운트다운 UI(`StartCountdownUI`)는 **Tutorial에서만** 연결 해제 — 스테이지 시작 게이트·Interlude는 기존 UI 그대로.

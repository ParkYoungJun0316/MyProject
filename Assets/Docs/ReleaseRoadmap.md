...
### 1.1a 빌드 검수 타이밍 (2026-08-06 결정)

- 9/1 고정 + Coming Soon **2주** 노출 필요 → 늦어도 **8/18**에는 페이지(Coming Soon)가 열려 있어야 함. 2026-08-06 기준 버퍼 **12일**뿐 — 빌드 검수 1차 실패(+~7일) 시 버퍼가 거의 소진됨.
- **빌드 검수는 콘텐츠 완성도를 요구하지 않음** — 실행되고 크래시 없고 스토어 정책 위반이 없으면 통과 대상. **옵션/설정 메뉴·BGM·SFX·애니메이션 완성을 기다리지 않고, Depot 실사용 2인 스모크 통과 직후 즉시 제출**한다.
- 검수 대기 시간(Valve 처리, 외부 요인)과 콘텐츠 작업 시간(응원 확장 테스트 → BGM → 옵션 → SFX → 애니)을 **병행**해서 쓴다. 검수 결과가 나온 뒤 콘텐츠가 더 채워지면 업데이트 빌드로 재업로드.

### 1.2 주간 초점 (러프)

| 구간 | 초점 |
|------|------|
| **지금 (8/6)** | Steamworks 코드(트랙1~3)·다국어 완료 → **Depot 실사용 2인 스모크** → **빌드 메타 + 빌드 검수 즉시 제출**(§1.1a, 최우선) |
| **검수 대기 병행** | 응원 확장 2종 테스트 → BGM → 옵션/설정 메뉴 → SFX+볼륨조절 → **애니**(트레일러) |
| **중반** | Tutorial |
...
- **Steamworks (전부):** Steam Networking transport + Steam Lobby + Depot/알파 빌드 + Invite — §3 ④ · §4 순위 1~3(코드 완료, Depot 실사용 테스트·빌드 검수 진행 중). **로컬 NGO만으로 출시 불가**
...
```
[지금]            Depot 2인 스모크 → 빌드 메타+빌드 검수 즉시 제출 (최우선, §1.1a)
[검수 대기 병행]   응원 확장 테스트 → BGM → 옵션/설정 메뉴 → SFX+볼륨조절 → 애니(트레일러)
[중반]            Tutorial

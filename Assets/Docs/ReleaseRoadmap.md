...
**목표 하나:** **2026-09-16 Steam 정식 출시.** (9/1 → 9/8 → 9/16, 아래 참고)

> **🔶 출시일 변경 (2026-09-11 결정):** 9/8 → **9/16(수)**로 재연장. 사유는 이 세션에서 별도 기록 없음 — 필요 시 다음 세션에서 사유 보강.
> **🔶 출시일 변경 (2026-08-23 결정):** 9/1 → **9/8(화)**로 1주 연장. 남은 콘텐츠 작업(§4 순위 5~11) 여유 확보 목적. 요일은 원래(화)와 동일 유지 — Tue~Thu 권장 요일 룰 유지, 금(9/4)·월(9/7)은 배제.
> **⚠️ Steamworks 조치 필요:** 출시일이 특정일 기준 14일 이내로 들어오면 Steamworks에서 직접 수정 불가(잠김). **Steam 파트너 지원팀에 티켓 제출해 9/16으로 변경 요청 필요** — 아직 미제출이면 최우선 처리.
...
| 항목 | 값 |
|------|-----|
| **출시일** | **2026-09-16** (정식, 2026-09-11 결정 — 9/1 → 9/8 → 9/16) |
| **배포** | Steam 정식 Depot만. 데모 페이지·**Steam Playtest Depot 없음** |
| **스토어 상태 (현재)** | **페이지·빌드 검수 통과, 출시일 9/16으로 재조정 (Steam 지원팀 변경 요청 필요)** |
...
페이지 검수 (완료)
  ...
  → 2026-09-16 정식 출시 (2026-09-11 결정, 9/1 → 9/8 → 9/16)
```
...
| **9/16** | 정식 출시 버튼 (9/1 → 9/8 → 9/16, 총 2주 연장) |

### 1.1a 빌드 검수 타이밍 (2026-08-06 결정) / 출시일 변경 (2026-08-23, 2026-09-11 결정)
...
- **9/8 → 9/16 변경 (2026-09-11):** 추가 연장. 요일은 **수요일**로 이동(Tue~Thu 권장 요일 룰 내).
- **Steamworks 조치:** 출시일 14일 이내 진입 시 Steamworks 랜딩 페이지에서 직접 수정 불가 → **Steam 파트너 지원팀에 티켓 제출해 9/16으로 변경 요청** 필요 (미완료 시 최우선 처리).
- Coming Soon 2주 노출 조건: 9/16 기준으로는 늦어도 **9/2**에 Coming Soon이 열려 있어야 함. 실제 오픈일 확인 필요.

### 1.2 주간 초점 (러프)

| 구간 | 초점 |
|------|------|
| **지금 (9/11)** | 빌드 검수 통과 완료, 출시일 9/8→9/16 변경 결정 → **콘텐츠 마무리** (응원 확장 테스트 → BGM → 옵션/설정 메뉴 → SFX+볼륨조절 → 애니) |
| **중반** | Tutorial |
| **Coming Soon 2주** | **난이도** (play test 피드백) · 버그 핫픽스 · QA 준비 |
| **출시 직전** | **출시 QA** → 9/16 |
| **출시 후 OK** | **텔레메트리** (9/16 블로커 아님) |
...
| 항목 | 처리 |
|------|------|
| **텔레메트리** | **출시 이후 착수 OK** (9/16 블로커 아님). 스펙: [`TelemetryDesign.md`](TelemetryDesign.md) |
...
```
[완료]            Depot 2인 스모크 → 빌드 메타+빌드 검수 → ✅ 통과, 출시일 9/1→9/8→9/16 변경 결정 (2026-08-23, 2026-09-11)
...
| 19 | **출시 QA** | §6 |
| 20 | 스크린샷·스토어 최종 | |
| 21 | **2026-09-16 정식 출시** | 9/1 → 9/8 (2026-08-23 결정) → 9/16 (2026-09-11 결정) |

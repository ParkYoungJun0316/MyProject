...
- `GridTileCollapse`(Random 모드) — 라운드 시작(안전 칸 확정) 뒤 **안전 칸이 아닌 칸만** 깬다. 안전 칸은 절대 안 깨짐.
- 깰 칸 수 = 인스펙터 커브(지금 1–4R 3칸 / 5–8R 5칸 / 9R+ 7칸). 남은 칸이 4방향 한 덩어리 + 안전 칸 전부 포함이 되도록 같은 시드로 재추첨.
- **순차 파괴**: 시드로 섞은 무작위 순서(위치 순서 아님) 1칸씩 0.3s 간격, 첫 파괴 1.0s, 칸마다 0.8s 전 노랑→빨강 경고. 안전 칸 주변이 다 깨질 수 있는 건 감안(사용자 확정).
- 정산 순간 전부 복구(파편 되감기 0.8s + ReverseTime 1회). 떨어지면 **즉사**(공용 `fallDeathY`).
- 개인 데미지 1 → **2**. 새 RPC/NV 없음 — 라운드 시드로 전 머신 동일 재생.
- 씬: 칸마다 자식 `Solid`(5×5 고체) + 통짜 `Ground` 콜라이더 비활성. 같은 컴포넌트를 T.Boss P3가 `AllExceptSafe`로 쓴다(`CoopStageAudit.T.md` §6).

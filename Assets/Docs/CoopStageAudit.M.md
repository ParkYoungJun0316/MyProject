...
2. **할 일:** ColorTile 점수제 §3 — 코드 됨. 타이머·발동 스케줄·실패 패널티 **폐기**. **에디터:** M.Stage3 `uniqueQuota`=6, 흑·백 배열 1인 4/4 · 2인 6/6 · 3인 7/7 · 4인 8/8, `tilePrefabs`에 Black·White, `ColorTileRoundObjective.targetTime`=180. Barrier §2.1 슬롯 코드 됨(2026-09-05) — M.Stage1 이동은 에디터 남음. **Barrier incoming = §2.2 감독 — `ArrowIncomingDirector` + `ArrowTrap.FireOnce()` 코드 됨(2026-09-05). 에디터(사용자) 남음: §2.2 표 참고.** Sequence 4.1 인원별 배열 에디터 됨(40/75/85/95 · 35/55/65/75). 4.2 스텝=초 됨(35/65/70/75). **Grid 룰 = §8 (2026-09-11 잠금, 코드 됨 — H.4 참고). 에디터 배선 남음.**
...
| M.Boss | **4페이즈** §7 (2026-09-08) | 초출 금지 | 1–3 복습, **4 입 닫힘+타일 파괴 5회(20→24개) → 삼켜 T** | 시드=Host ChallengeStart | Grid·Sequence·ColorTile·WindTrap 없음. P2는 SideSplit(2026-09-21 문서 정정). 바람만 페이즈 없음. 혀 MixedSweep도 보스에서 뺌 |
...
| 3 | Drop + 화살 + 혀 — 혀가 장면 | 혀 |
| 4 | 입 닫힘(무조건) + 타일 파괴(파괴음), 5회(20→24개) → 삼켜 T | 닫힘(보스 전용 새 변형) |
...
5. 다음 회차 경고로 이어짐. 총 **5회**. **마지막(5)회차는 응원 창을 열지 않는다**(2026-09-21 확정) — Open 직후 곧장 클리어 → Bossdown 대화가 **1칸 남은 바닥 위에서** 나온다. 창을 열면 응원 성공 시 25칸이 복구된 채 엔딩 대화가 나왔다(플레이테스트).

**크기 누적(회차 번호 기준, 응원 성공 여부와 무관) [2026-09-23 개정 — 난이도 상향]:** 회차 N의 목표 파괴 수 = **20 + 1×(N-1)**(1=20, 2=21, 3=22, 4=23, 5=24 — 남는 칸 5→4→3→2→1). 인스펙터 `firstCycleBreakCount`=20 · `tilesPerCycleStep`=1 · `totalCycles`=5. 직전 회차가 복구됐으면 이번에 목표 개수를 전부 새로 뽑고, 복구 안 됐으면 이미 깨진 개수에 **1개만** 추가로 뽑는다(응원은 항상 전체 복구라 회차 사이 깨진 수는 0 또는 직전 목표만 나옴 — 산수가 갈라지지 않음). **총 타일 수 = 25** — "총 타일 = 1회차 목표 + 증가분×(회차수−1) + 1" 관계는 유지해야 "마지막 1칸" 결말이 보장됨. 연속으로 여러 회차를 놓쳐서 남은 칸이 목표보다 적으면 남은 칸 전부를 깨는 걸로 캡. `ValidateWiring()`이 이 불변식과 배열 길이 불일치를 Awake에서 경고.
- **왜 (2026-09-23):** 옛 4×N은 1회차 21칸이 안전해 "한 걸음 옆으로"가 전부였고 암전 동안 할 일이 없었다. 첫 회차부터 80%를 깨 매 회차 남은 칸을 찾아 뛰게 한다.
- **추첨은 순수 랜덤 [확정]:** 남는 칸이 덩어리로 붙거나 8방향으로 이어지도록 보정하지 **않는다**. 흩어져 섬이 되는 게 의도 — 응원을 안 하면 다음 회차에 1칸만 더 깨지지만, 그게 내가 선 섬이면 건너갈 곳 없이 떨어진다. 그래서 응원이 사실상 필수가 된다(응원 실패 벌칙을 따로 두는 안은 이것으로 대체, 폐기).

**클리어:** 마지막(5)회차가 끝나면(성공/실패 무관, 팀이 살아있으면) 클리어. 시간 기반 `PhaseSurviveChallenge`가 아니라 **회차 카운트 기반** — `MouthBossJawSmash`가 5회 완료 시 `BossFightObjective.NotifyPhaseCleared()`를 직접 호출(Host 레인 가드, 기존 챌린지들과 동일 연결 방식).
...
**엔딩:** 클리어 → 대화(Bossdown) → 마지막 남은 1칸까지 파괴음과 함께 부서지는 연출 → T로 전환. **남은 칸만 부순다(2026-09-23):** `ForceBreakAllTilesForEnding()`은 살아 있는 칸에만 파괴음(`Breakable_Destroy`)+파편을 낸다 — 예전엔 이미 빈 24칸에도 타일 모양 파편을 스폰해 "25칸이 복구됐다가 동시에 깨지는" 그림이었다. 클리어 후 24칸 복구→대화→전부 파괴 안은 #12b(1칸 위 대화) 결정과 충돌해 기각. 원래 잠긴 "혀가 바닥을 부숨 → 삼켜 T"는 폐기 — 주체가 혀에서 입(파괴음 기반)으로 바뀜.
...
난이도 원인(사용자 진단, PlaytestLog #3): 데미지원이 라운드 정산뿐인데 실패 여지가 1~2라운드, 방해는 약하고 짧은 바람 하나, 4인은 부활 3회로 라운드를 무시해도 됨. 그래서 **바람을 세게 하지 않고 "밀리면 죽는" 바닥**을 더한다.

...
| 4종 | **별 · 웃는 얼굴 · 음표 · 꿀떡**(`Assets/Figma/Ingame/M2/{Star,Face,Note,Heart}.png`). 파일명 `Heart`는 하트가 아니라 **꿀떡 모양**이다 — 하트 패널의 "하트"도 실제로는 꿀떡이라 이름만 겹치고 그림은 겹치지 않는다 |
...
| 담당 | **모양 ↔ 설계슬롯 4색 1:1 고정** — 별=Blue, 얼굴=Purple, 음표=Green, 꿀떡=Yellow. 판 내내는 물론 **매 판 동일**하다. 시드를 쓰지 않는다. 씬 배선은 `DirectionView.ownerColor`(간판 배치: FRONT=별, RIGHT=얼굴, BACK=음표, LEFT=꿀떡 — `M.Stage2`·`M.Boss` 동일) |
...
- [ ] Tip 한 줄 — `M.Stage2`가 SideSplit 초출이라 비대칭은 가르쳐야 함 ([`StageTipLines.md`](StageTipLines.md)). 문구는 "네 색 모양의 하트만 보인다" 한 줄이면 족함

...
- **✅ 영구화 완료 (2026-08-17):** ParrelSync 클론에서 이 캐시 위치 이슈가 실제로 재발(클론이 별도 `Library`를 새로 만들면서 업스트림의 미수정 버전을 다시 받아와 `CS1028` 재현)하여, 위 3가지 수정(중복 `#endregion` 삭제, `Shutdown()`의 `SteamClient.Shutdown()` 제거, 아래 virtual port 필드/적용)을 모두 담은 `Library/PackageCache` 사본을 `Packages/com.community.netcode.transport.facepunch/`로 **Embed**하고, `Packages/manifest.json`의 git URL 의존성을 로컬 버전(`"2.0.0"`) 참조로 교체함. **메인 프로젝트 + `My project_clone_0` 양쪽 모두 적용** — 클론의 `Packages` 폴더는 `Assets`/`ProjectSettings`와 달리 심볼릭 링크가 아니라 별도 폴더라서 동일하게 복사·수정함. 이후 `Library` 삭제/재-resolve와 무관하게 두 프로젝트 모두 유지됨.
...
- `Library/PackageCache/com.community.netcode.transport.facepunch@27d3e825ecdd/Runtime/FacepunchTransport.cs`: `virtualPort` 필드 추가, `StartServer()`의 `CreateRelaySocket()`과 `StartClient()`의 `ConnectRelay()`에 전달하도록 수정. (§2/이슈 D 3차 세션 수정과 같은 성격의 임시 위치 이슈 — Package Manager "Embed" 시 함께 영구화 필요.) **✅ 영구화 완료 — §2 참고(2026-08-17).**

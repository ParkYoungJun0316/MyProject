...
| 씬 클리어 확정 후 사망 (모든 Phase 완료 ~ 다음 씬) | **실패 없음**(2026-09-23). 목숨이 남았으면 위와 같이 부활, 없거나 생존자 0이면 죽은 채 전환을 기다린다. ESC Reset도 무시. 보스(M/T) Bossdown 대화 중 낙사가 보스 전체 리로드로 이어지던 구멍을 막음 — 일반 스테이지는 클리어와 전환이 같은 순간이라 원래 이렇게 동작했다. `StageNetworkState.BeginStageResetOnServer` 가드 |
